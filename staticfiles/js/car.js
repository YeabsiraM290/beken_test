...
var base_url = `${host}/api/car/?${getParameterByName('filters')?getParameterByName('filters'):''}`

function filter() {
    console.log('chage')
    current_page = 1
    let transmission_choice = document.getElementById('transmission').value
    ...
    base_url = `${host}/api/car/?`
    filters = `${transmission_choice ?"transsmission=".concat(transmission_choice).concat('&'):''}${price_min?"min_price=".concat(price_min).concat('&'):''}${price_max?"max_price=".concat(price_max).concat('&'):''}${year_start?"from_year=".concat(year_start).concat('&'):''}${year_end?"to_year=".concat(year_end).concat('&'):''}`
    ...
    get_data().then(function(data) {
            ...
            if (data.length > 0) {
                ...
                <div class="card-sl">
                    ...
                    <div class="row text-center feature-list  mb-3">
                        ...
                        <div class="col-md"><b>Brand: </b> ${property.more['brand']}</div>
                        <div class="col-md"><b>Model: </b>${property.more['car_model']}</div>
                    

                    </div>
                    <div class="row text-center feature-list mb-3">
                        <div class="col-md"><b>Year: </b>${property.more['year']}</div>
                        <div class="col-md"><b>Condition:</b> ${property.more['condition']}</div>
                    </div>

                    <div class="row text-center feature-list  mb-3">

                        <div class="col-md"><b>Transmission: </b>${property.more['transmission']}</div>
                        <div class="col-md"><b>Price: </b>${property.price} birr</div>
                    </div>
            ...
            for (let i = 1; i <= page_no; i++) {
                ...
                if (i == current_page) {
                    ...
                    pagination_no += `<a  href="car?${filters}page=${current_page+1}">${i}</a>`

                } else if (i == current_page - 1) {
                    pagination_no += `<a href="car?${filters}page=${current_page-1}">${i}</a>`

                } else {
                    pagination_no += `<a  href="car?${filters}page=${i}">${i}</a>`
                }
